...
```go
// order_test.go
func TestOrder_Confirm_FromDraft_Success(t *testing.T) {
    // Draft状態の注文を作成し、Confirm()が成功することを確認
}

func TestOrder_Confirm_FromPaid_Error(t *testing.T) {
    // Paid状態の注文に対してConfirm()を呼ぶとエラーになることを確認
}

func TestOrder_Cancel_FromShipped_Error(t *testing.T) {
    // Shipped状態の注文はキャンセルできないことを確認
}

func TestOrder_Confirm_EmptyLines_Error(t *testing.T) {
    // 注文明細が0件の注文は確定できないことを確認（不変条件）
}
```

すべての状態遷移についてテストを書き、ビジネスルールが正しく実装されていることを確認してください。
